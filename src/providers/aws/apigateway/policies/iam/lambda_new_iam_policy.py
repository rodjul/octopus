...
def lambda_handler(event,context):
    ...
    if not UserACL(username).has_acl("aws","create-iam-policies"):
    ...
    for policy in policies:
        ...
        if exist_policy("POLICY", policy_name):
            return {"statusCode":400, "body":dumps({"error":True, "message":"IAM Policy already exist"}),
            "headers":{ "Content-Type":"application/json", "Access-Control-Allow-Origin":"*"}}
    ...
    for trust in trusts:
        ...
        if exist_policy("TRUST", policy_name):
            return {"statusCode":400, "body":dumps({"error":True, "message":"IAM Trust already exist"}),
            "headers":{ "Content-Type":"application/json", "Access-Control-Allow-Origin":"*"}}
    ...
    for role in roles:
        ...
        if exist_policy("ROLE", policy_name):
            return {"statusCode":400, "body":dumps({"error":True, "message":"IAM Role already exist"}),
            "headers":{ "Content-Type":"application/json", "Access-Control-Allow-Origin":"*"}}

        if not valid_role(role):
            return {"statusCode":400, "body":dumps({"error":True, "message":"IAM Role contains invalid data"}),
            "headers":{ "Content-Type":"application/json", "Access-Control-Allow-Origin":"*"}}
